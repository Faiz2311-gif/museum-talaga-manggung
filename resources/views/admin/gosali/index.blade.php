            ...
            <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm text-stone-600">
                        Kelola urutan konten dokumenter, sinopsis babak, dan lampiran panduan untuk halaman Gosali agar publik bisa menonton dan membaca dengan konsisten.
                    </p>
                </div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('gosali') }}" class="rounded-lg border border-amber-200 bg-white px-4 py-2.5 text-xs font-semibold text-amber-800 transition hover:bg-amber-50">
...
<!-- Bagian Editor Header Dinamis Khusus Admin Gosali -->
<div class="mb-8 p-6 bg-white border border-stone-200/80 rounded-2xl shadow-sm">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div class="flex items-start sm:items-center gap-4 flex-1">
            <div class="shrink-0 relative group">
                <!-- Membaca variabel array global $banners dengan indeks 'gosali' -->
                @if(isset($banners) && isset($banners['gosali']))
                    <img src="{{ asset('storage/' . $banners['gosali']) }}" 
                         alt="Header Gosali Saat Ini" 
                ...
                @else
                    <div class="w-32 h-20 bg-stone-100 rounded-xl border border-dashed border-stone-300 flex flex-col items-center justify-center text-center p-2">
                        <svg class="h-5 w-5 text-stone-400 mb-1" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H2.25A1.5 1.5 0 00.75 6v12.75a1.5 1.5 0 00.75 1.5z" />
                        </svg>
            ...
            <div>
                <h3 class="text-sm font-bold text-stone-800 flex items-center gap-1.5">
                    <span>Banner Header Gosali</span>
                ...
                <p class="text-xs text-stone-500 mt-0.5 max-w-md leading-relaxed">
                    Gambar ini akan muncul sebagai latar belakang judul utama pada halaman informasi / pandai besi Gosali publik.
                </p>
            </div>
        </div>

        <!-- Form Pemilihan & Aksi Simpan untuk Admin Gosali -->
        <form action="{{ route('admin.setting.update-header') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
            ...
            <!-- 🔴 PENANDA HALAMAN: Diatur khusus ke 'gosali' -->
            <input type="hidden" name="halaman" value="gosali">
            
            <div class="relative">
                <input type="file" name="banner_image" id="banner_gosali_input" accept="image/*" required
            ...
            <button type="submit" class="inline-flex items-center justify-center space-x-2 bg-stone-800 hover:bg-stone-900 text-white font-bold text-xs px-5 py-2.5 h-[38px] rounded-xl transition duration-150 shadow-sm">
                ...
                <span>Perbarui Banner Gosali</span>
            </button>
        </form>
        ...
        @enderror
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
                <div class="space-y-6 lg:col-span-2">
                    <div class="relative aspect-video overflow-hidden rounded-2xl border border-amber-900/10 bg-stone-950 shadow-xl">
                ...
                <div class="space-y-4">
                    <div class="overflow-hidden rounded-xl border border-amber-200/60 bg-white shadow-sm">
                        <div class="flex items-center justify-between bg-stone-900 p-4 text-amber-100">
                            ...
                            <span class="rounded bg-amber-800/80 px-2.5 py-1 font-mono text-xs">
                        ...
                        <div id="playlistList" class="max-h-[520px] divide-y divide-stone-100 overflow-y-auto">
                            @forelse($videos as $video)
                                ...
                                <div
                                    ...
                                    onclick="playVideo(this, '{{ $videoSource }}', {{ json_encode($video->title ?? '') }}, {{ json_encode($video->description ?? '') }}, {{ json_encode($video->duration ?? '') }}, '{{ $posterSource }}', '{{ $youtubeEmbedUrl }}')"
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-start justify-between gap-2">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('admin.gosali.edit', $video->id) }}" class="text-[10px] font-semibold text-amber-700">✏️</a>
                                                <form action="{{ route('admin.gosali.destroy', $video->id) }}" method="POST" onsubmit="return confirm('Hapus video ini?')">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div id="modalTambahVideo" class="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 p-4 opacity-0 invisible transition-all duration-300 backdrop-blur-sm">
        <div class="w-full max-w-lg scale-95 rounded-xl border border-amber-200 bg-white shadow-2xl transition-all duration-300">
            ...
            <form action="{{ route('admin.gosali.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 p-6 text-xs text-stone-700">
                @csrf

                <div>
                    <label class="mb-1 block font-bold uppercase tracking-wider text-stone-600">Judul Babak Video</label>
                    <input type="text" name="title" placeholder="Contoh: 1. Kisah Gosali" class="w-full rounded-lg border border-stone-200 bg-stone-50/50 px-3 py-2 focus:border-amber-600 focus:outline-none" required />
                </div>
    ...
    <script>
        ...
        function playVideo(element, videoSrc, title, description, duration, posterSrc, youtubeEmbedUrl) {
            ...
            if (player) {
                if (youtubeEmbedUrl) {
                    player.outerHTML = '<iframe id="mainMuseumPlayer" class="h-full w-full" src="' + youtubeEmbedUrl + '" title="' + title + '" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
                } else {
                    player.src = videoSrc || '';
        ...
        function dropItem(event, targetId) {
            ...
            fetch('{{ route('admin.gosali.reorder') }}', {
                method: 'POST',
                headers: {
